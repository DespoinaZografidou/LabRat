@if(count($chooseNotifications)!=0)
    <div class="table">
        <div class="table_tbody">
            @foreach ($chooseNotifications as $s)
                <div class="cn table_tr_ch time-row" data-start='{{$s->created_at}}' data-end='{{$s->updated_at}}' data-info="<?php echo htmlspecialchars(json_encode((array)$s), ENT_QUOTES, 'UTF-8'); ?>"  @if(strpos($s->msg, 'αποδεσμεύτηκε')!== false) style="background-color:rgba(255, 0, 0, 0.1)"  @endif>
                    <div class="td" style="width: 95%;">
                        ...
                        <form method="POST" action="{{url('/deletechooseannounsements')}}">@csrf @method("GET")
                        ...
                        @if($s->msg==='')
                            <p class="pl-2">Δεσμεύτηκε το θέμα <b>"{{$s->th_title}}"</b> από την εσάς.</p>
                        @elseif(strpos($s->msg, 'αποδεσμεύτηκε') !== false)
                            <p class="pl-2">Αποδεσμεύτηκε το θέμα <b>"{{$s->th_title}}"</b>
                        ...
                        @else
                            <p class="pl-2">Δεσμεύτηκε το θέμα <b>"{{$s->th_title}}"</b> από την ομάδα σας, με τα μέλη:<br>  {!!str_replace( ',',', ' ,$s->msg)!!}</p>
                        @endif
    ...
    {{ $chooseNotifications->links('pagination::bootstrap-4') }}
@endif
@if(count($chooseNotifications)==0)
...
<script>
    $(document).ready(function () {
        ...
        $link.click(function (event) {
            ...
            let url = "/showTheThemes/" + myOb.l_id + "/" + myOb.l_title + "/" + myOb.name + "/" + myOb.act_id;
