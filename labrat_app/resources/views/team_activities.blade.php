            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header" style="display: flex;justify-content: space-between;">
                        <h5>Δραστηριότητα Ομάδες</h5>
                        @if(Auth::user()->role!='Μαθητής')
                            <div class="text-end">
                                <button type="button" class="button-3" onclick="addNotification()">+Νέα δραστηριότητα</button>
                            </div>
                        @endif
                    </div>
                        ...
                        @if(count($acteams)!=0)
                            ...
                            <div class="table">
                                <div class="table_tbody">
                                    @foreach($acteams as $a)
                                        <div class="table_tr_ch time-row" data-end='{{$a->updated_at}}' data-start='{{$a->created_at}}'  data-role="{{Auth::user()->role}}"  data-info="<?php echo htmlspecialchars(json_encode((array)$a), ENT_QUOTES, 'UTF-8'); ?>">
                                            <div class="td" style="display: flex;">
                                                <div class="nt">
                                                    <div  class="at_open">
                                                        <h6>
                                                            <button type="button" class="status"></button>
                                                            <a href="{{ route('showTheTeams',['l_id'=>$l_id,'title'=>$title,'name'=>$name,'at_id'=>$a->id]) }}">{{$a->title}}</a>
                                                        </h6>
                                                    </div>
                                                    ...
                                                    <div class="pt-3" style="text-align: right; display: flex; justify-content: space-between"><p class='time-difference'></p><p>Λήξη: {{date('d-m-Y H:i', strtotime($a->updated_at))}} </p></div>
                                                </div>
                                                @if(Auth::user()->role!='Μαθητής')
...
{{--                            the links for the next page of results--}}
                            {{ $acteams->links('pagination::bootstrap-4') }}
                        @endif
                        @if(count($acteams)==0)
...
{{--    Pop up window to delete, edit or add a lesson--}}
    @include('popup-windows.activityTeam-Delete_Add_Edit')
    <script>
        $(document).ready(function () {
            // Hide the textarea initially
            $('#activitiesteam').addClass('active');
        });
