...
<div class="modal popup" id="popupdelete">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="post" action="/deleteActivityTeam">
                @csrf @method('GET')
                <div class="modal-body ">
                    <label class="col-md-12 col-form-label" id="text"></label>
                    <input type="text" id="at_id" name="at_id" value="" hidden>
...
<div class="popup modal" id="popupedit">
    <div class=" modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ url('/createAddActivityTeam') }}" >
                    @csrf @method('GET')
                    ...
                    <div class="row mb-2">
                        <div class="col-md-6">
                            <label class="col-form-label text-md-start" for="text">Αριθμός μελών κάθε Ομάδας</label>
                            <select class="form-control" type="text" id="maxnum" name="maxnum" required >
                            ...
                            <div class="text-end">
                                <label class="col-form-label-sm">Αρ. Συμμετεχόντων Μαθήματος: {{$p_count}}</label>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function addNotification(){
        $("#popupedit").fadeIn();
        document.getElementById("demoModalLabel").textContent = 'Εισαγωγή Νέας Δραστηριότητας Ομάδας';
        ...
        document.getElementById("maxnum").options[0].text='--Επιλέξτε--';
        // document.getElementById("maxnum").textContent='--Επιλέξτε--';
    ...
    $link.click(function (event) {
        ...
        if (userRole === 'Μαθητής') {
            ...
            var title = "{{ $title }}";
            var name = "{{ $name }}";
            var at_id = myObject.id;

            var url = "/showTheTeams/" + encodeURIComponent(l_id) + "/" + encodeURIComponent(title) + "/" + encodeURIComponent(name) + "/" + at_id;
        ...
        if (userRole === 'Καθηγητής' ||userRole === 'Διαχειριστής' ) {
            ...
            if ($button.length) {
                ...
                document.getElementById('at_id').value = myObject.id;
            ...
            document.getElementById("demoModalLabel").textContent = 'Ενημέρωση Δραστηριότητας Ομάδα';
            ...
            document.getElementById("maxnum").value=myObject.mnp;
            document.getElementById("maxnum").options[0].text=myObject.mnp;
