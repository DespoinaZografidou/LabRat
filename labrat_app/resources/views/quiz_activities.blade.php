...
@section('content')
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header" style="display: flex;justify-content: space-between;">
                        <h5>Δραστηριότητα Κουίζ</h5>
                        @if(Auth::user()->role!='Μαθητής')
                            <div class="text-end">

                                <button type="button" class="button-3 newactivity" >+Νέα δραστηριότητα</button>
                            </div>
                        @endif
                    </div>
                    <div class="card-body results" id="results">
                        ...
                        @if(count($act_quiz)!=0)
                                            ...
                                            <div class="td" style="display: flex;">
                                                <div class="nt">
                                                    <div class="at_open" >
                                                        <h6>
                                                            ...
                                                            <a href="{{ route('showTheQuiz',['l_id'=>$l_id,'title'=>$title,'name'=>$name,'act_id'=>$a->id]) }}">{{$a->title}}</a>
                                                            <div style="display: flex;justify-content: space-between">
                                                                <p class="ml-3 mt-2"> Αρ. προσπαθειών: @if(Auth::user()->role==='Μαθητής')<b>{{$a->allthetries}}</b>@endif/{{$a->tries}}</p>
                                                                @isset($a->finalgrade)
                                                                <p>Βαθμολογία: <b>{{$a->finalgrade}}</b>/{{$a->maxgrade}}</p>
                                                                @endisset
                                                            </div>
                                                ...
                                                @if(Auth::user()->role!='Μαθητής')
                                                    <div class="nd" style="position: absolute;right: 20px;">
                                                        <button type="button" class="delete"><span class="delete__icon"><ion-icon name="trash"></ion-icon></span>
                                                        </button>
                                                    </div>
                                                @endif
                            ...
                            {{ $act_quiz->links('pagination::bootstrap-4') }}
                        @endif
                        @if(count($act_quiz)==0)
    ...
    {{--    Pop up window to delete, edit or add a lesson--}}
        @include('popup-windows.activityQuiz-Delete_Add_Edit')
    <script>
        $(document).ready(function () {
            // Hide the textarea initially
            $('#quizactivities').addClass('active');
        });
